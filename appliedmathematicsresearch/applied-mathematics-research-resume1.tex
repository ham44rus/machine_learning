...
\begin{block}{命題 7.2}
$W \sim W_p(m)$ ならば $\text{tr } W \sim \chi^2_{mp}$。
\end{block}
\begin{block}{証明}
$W \sim W_p(m)$ の定義より、$\text{tr } W \overset{d}{=} \text{tr } \sum_{i=1}^{m} z_i z_i' = \sum_{i=1}^{m} z_i' z_i$。\\
$z_i$ は独立同分布で $N_p(0, I)$ に従うので、命題4.4より $z_i' z_i \sim \chi^2_p$。\\
系3.1より $\text{tr } W \sim \chi^2_{mp}$。
\end{block}
\end{frame}

\begin{frame}{7.3 Wishart分布の性質 (続き)}
\begin{block}{補題 7.1}
$Z = (z_{ij}) \in R_n^n$ が独立同分布の $N(0, 1)$ に従う場合、$P(|Z| = 0) = 0$。
\end{block}
\begin{block}{証明}
...
\end{equation*}
$Z_{22} \in R_{n-1}^{n-1}$ に対して結果が成立すると仮定すると、$P(|Z|=0) = P(|Z|=0, |Z_{22}| \ne 0) + P(|Z|=0, |Z_{22}|=0)$ 
\begin{align*}
...
\begin{frame}{7.3 Wishart分布の性質 (続き)}
\begin{block}{系 7.1}
$Z = (z_{ij}) \in R_n^n$ が独立同分布の $N(0, 1)$ に従う場合、$P(|Z| = t) = 0, \forall t$。
\end{block}
\begin{block}{証明}
